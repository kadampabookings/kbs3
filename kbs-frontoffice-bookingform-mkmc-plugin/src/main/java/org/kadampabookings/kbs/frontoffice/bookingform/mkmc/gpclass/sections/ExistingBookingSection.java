...
import one.modality.booking.frontoffice.bookingpage.sections.HasMemberSelectionSection.MemberStatus;
...
/**
 * Section for handling existing booking choice flow.
 ...
 * @author Claude
 */
public class ExistingBookingSection implements BookingFormSection {

    /**
     * Selection type for the existing booking section.
     */
    public enum SelectionType {
        /** User wants to modify an existing booking */
    ...
    private static final String ICON_CHECK = "M20 6L9 17l-5-5";
    ...
    // === CALLBACKS ===
    private Consumer<SelectionType> onSelectionTypeChanged;
    ...
    private void buildUI() {
        ...
        HBox existingBookingsHeader = new StyledSectionHeader(
                MKMCI18nKeys.GPExistingBookingsModifySection,
        ...
        HBox otherMembersHeader = new StyledSectionHeader(
                MKMCI18nKeys.GPOtherMembersSection,
    ...
    private HBox buildInfoNoticeBox() {
        ...
        Label titleLabel = I18nControls.newLabel(MKMCI18nKeys.GPExistingBookingsFound);
        titleLabel.getStyleClass().addAll("bookingpage-text-base", "bookingpage-font-semibold", "bookingpage-text-dark");

        Label descLabel = I18nControls.newLabel(MKMCI18nKeys.GPExistingBookingsInfoText);
    ...
    private HBox buildValidationMessageBox() {
        ...
        Label messageLabel = I18nControls.newLabel(MKMCI18nKeys.GPPleaseSelectOption);
    ...
    private HBox buildButtonRow() {
        ...
        I18n.bindI18nTextProperty(continueButton.textProperty(), MKMCI18nKeys.GPModifyBooking);
        ...
        selectedAction.addListener((obs, oldAction, newAction) -> {
            ...
            if (newAction != null && newAction.startsWith("modify-")) {
                ...
                if (onSelectionTypeChanged != null) {
                    onSelectionTypeChanged.accept(SelectionType.MODIFY_EXISTING_BOOKING);
                }
            } else if (newAction != null && newAction.startsWith("new-")) {
                I18n.bindI18nTextProperty(continueButton.textProperty(), MKMCI18nKeys.GPStartNewBooking);
                if (onSelectionTypeChanged != null) {
                    onSelectionTypeChanged.accept(SelectionType.CREATE_NEW_BOOKING);
                }
            }
    ...
    private VBox createExistingBookingCard(BookingInfo info) {
        ...
        if (info.isPrimary()) {
            Label youBadge = I18nControls.newLabel(MKMCI18nKeys.GPYouBadge);
        ...
        if (hasPendingPayment) {
            HBox statusBadge = createStatusBadge(MKMCI18nKeys.GPStatusPendingPayment, false);
        ...
        // Reference
        VBox refBox = createSummaryItem(MKMCI18nKeys.GPReference,
                info.getDocumentRef() != null ? "#" + info.getDocumentRef() : "-");

        // Classes
        VBox classesBox = createSummaryItem(MKMCI18nKeys.GPDates,
                I18n.getI18nText(MKMCI18nKeys.GPClassesBooked, info.getClassesBooked()));

        // Payment
        String paymentText = formatCurrency(info.getAmountPaid()) + " / " + formatCurrency(info.getAmountTotal());
        VBox paymentBox = createSummaryItem(MKMCI18nKeys.GPPayment, paymentText);
        ...
        Label hintLabel = I18nControls.newLabel(MKMCI18nKeys.GPSelectToModify);
    ...
    private VBox createMemberCardNoBooking(MemberInfo member) {
        ...
        if (isBookable) {
            ...
            Label noBadgeLabel = I18nControls.newLabel(MKMCI18nKeys.GPNoBookingYet);
            ...
            // Action hint
            Label hintLabel = I18nControls.newLabel(MKMCI18nKeys.GPSelectToCreate);
            hintLabel.getStyleClass().addAll("bookingpage-text-xs", "bookingpage-text-muted");
    ...
    private void loadData() {
        ...
        if (userPerson == null) {
            Console.log("ExistingBookingSection: No user person, skipping data load");
            return;
        }
        ...
        if (principal == null) {
            Console.log("ExistingBookingSection: No user principal");
            return;
        }
    ...
    public Object getTitleI18nKey() {
        return MKMCI18nKeys.GPExistingBookingTitle;
    }
    ...
    public SelectionType getSelectionType() {
        ...
        if (action.startsWith("modify-")) return SelectionType.MODIFY_EXISTING_BOOKING;
        if (action.startsWith("new-")) return SelectionType.CREATE_NEW_BOOKING;
        return null;
    }
     ...
     */
    public void setOnSelectionTypeChanged(Consumer<SelectionType> callback) {
