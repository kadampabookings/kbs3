...
final class KbsFrontOfficeLoginActivity extends ViewDomainActivityBase {
    ...
    private static final String INFO_SVG_PATH = "M 4.6 3.1 c 0 -0.4 0.1 -0.8 0.2 -1.2 c 0.2 -0.4 0.4 -0.7 0.7 -1 c 0.3 -0.3 0.6 -0.5 1 -0.7 c 0.4 -0.2 0.7 -0.2 1.2 -0.2 c 0.4 0 0.8 0.1 1.2 0.2 c 0.4 0.2 0.7 0.4 1 0.7 c 0.3 0.3 0.5 0.6 0.7 1 c 0.1 0.4 0.2 0.8 0.2 1.2 c 0 0.4 -0.1 0.8 -0.2 1.2 c -0.1 0.4 -0.4 0.7 -0.7 1 c -0.3 0.3 -0.6 0.5 -1 0.7 c -0.4 0.2 -0.7 0.2 -1.2 0.2 c -0.4 0 -0.8 -0.1 -1.2 -0.2 c -0.4 -0.2 -0.7 -0.4 -1 -0.7 c -0.3 -0.3 -0.5 -0.6 -0.7 -1 c -0.1 -0.4 -0.2 -0.8 -0.2 -1.2 z m 1.3 17.9 c -0.1 0.4 -0.3 1.2 0.3 1.2 c 0.1 0 0.3 -0.1 0.6 -0.2 c 0.2 -0.2 0.5 -0.4 0.8 -0.8 c 0.3 -0.4 0.6 -0.8 1 -1.3 c 0.3 -0.5 0.7 -1 1.1 -1.6 a 0.1 0.1 90 0 1 0.2 -0 l 1.2 0.9 c 0.1 0 0.1 0.1 0 0.2 c -0.6 1 -1.2 1.9 -1.7 2.6 c -0.6 0.7 -1.2 1.4 -1.8 1.9 l -0 0 c -0.6 0.5 -1.2 0.9 -1.9 1.1 c -1.8 0.7 -4.6 0.6 -5.4 -1.5 c -0.5 -1.3 -0.1 -2.8 0.3 -4.1 l 2 -6.1 c 0.1 -0.5 0.3 -1 0.4 -1.4 c 0.1 -0.8 -0.3 -1.3 -1.1 -1.3 h -1.8 a 0.1 0.1 90 0 1 -0.1 -0.1 l 0 -0 l 0.5 -1.7 a 0.1 0.1 90 0 1 0.1 -0.1 l 9 -0.3 a 0.1 0.1 90 0 1 0.1 0.1 l -0 0 l -3.8 12.6 z z";
    ...
    private final Media manjushriMedia = player.acceptMedia("wistia:z3pqhk7lhs");
    ...
    public Node buildUi() {
        ...
        SvgIcons.armButton(cornerButton, () -> {
            if (loginUI.getOpacity() == 1)
                showVideo(true);
            else if (player.getMedia() == droneMedia) {
                displayManjushriVideo();
                showVideo(true);
            } else
                showVideo(false);
        });
        ...
        player.setOnEndOfPlaying(() -> {
            Console.log("OnEndOfPlaying called");
            showVideo(false);
        });
    ...
    private void displayDroneVideo() {
        ...
        FXProperties.runOnPropertyChange(time -> {
            Console.log(time);
    ...
    private void displayManjushriVideo() {
        player.stop();
        player.setMedia(manjushriMedia);
        player.setStartOptions(new StartOptionsBuilder().setAutoplay(true).build());
        player.displayVideo();
        aspectRatioPane.setMouseTransparent(false);
        aspectRatioPane.setFitMode(ScaleMode.BEST_FIT);
    }
        ...
        if (show) {
            ...
            if (showingDroneScheduled == null || showingDroneScheduled.isFinished()) {
                showingDroneScheduled = UiScheduler.scheduleDelay(300, () -> {
                    cornerButton.setContent(player.getMedia() == droneMedia ? SvgIcons.createSVGPath(INFO_SVG_PATH) : SvgIcons.createSVGPath(EXIT_SVG_PATH));
            ...
            player.mute();
            if (player.getMedia() == manjushriMedia)
                player.pause();
