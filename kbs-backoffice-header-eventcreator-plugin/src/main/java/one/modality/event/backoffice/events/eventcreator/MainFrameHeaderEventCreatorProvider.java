...
public class MainFrameHeaderEventCreatorProvider implements MainFrameHeaderNodeProvider {
    ...
    private void openNKTFestivalCreatorDialog() {
        ...
        ToggleButton springButton = createEventTypeButton(EventCreatorI18nKeys.SpringCard, toggleGroup, FestivalType.SPRING_FESTIVAL);
        ToggleButton summerButton = createEventTypeButton(EventCreatorI18nKeys.SummerCard, toggleGroup, FestivalType.SUMMER_FESTIVAL);
        ToggleButton fallButton   = createEventTypeButton(EventCreatorI18nKeys.FallCard,   toggleGroup, FestivalType.FALL_FESTIVAL);
        ...
        //content.spacingProperty().bind(content.heightProperty().multiply(0.1)); // multiply not yet supported by webfx
        content.spacingProperty().bind(FXProperties.compute(content.heightProperty(), height -> height.doubleValue() * 0.1));
    ...
    private static ToggleButton createEventTypeButton(String cardI18nKey, ToggleGroup toggleGroup, FestivalType festivalType) {
        ...
        ObservableValue<Integer> nextYearProperty = FXProperties.compute(FXFestivals.lastFestivalProperty(festivalType),
            festival -> festival == null ? null : festival.getStartDate().getYear() + 1);
