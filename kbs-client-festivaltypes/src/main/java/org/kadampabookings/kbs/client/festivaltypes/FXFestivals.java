...
public final class FXFestivals {
    ...
    public static ReadOnlyObjectProperty<Event> lastFestivalProperty(FestivalType festivalType, boolean online) {
        if (festivalType != null) {
            return switch (festivalType) {
                case SPRING_FESTIVAL -> online ? lastOnlineSpringFestivalProperty() : lastSpringFestivalProperty();
                case SUMMER_FESTIVAL -> online ? lastOnlineSummerFestivalProperty() : lastSummerFestivalProperty();
                case FALL_FESTIVAL   -> online ? lastOnlineFallFestivalProperty()   : lastFallFestivalProperty();
            };
        }
        return null;
    }
